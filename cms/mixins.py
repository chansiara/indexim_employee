...
class DynamicTemplateMixin:
    ...
    def get_template_names(self):
        ...
        view_template =  "{}{}/{}_{}.html".format(app,self.subapp,model_name, self.template)
...
class AjaxCreateMixin:
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['object_list'] = self.get_queryset().all()[:25]
...
class AjaxUpdateMixin(ObjectMixin):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['object_list'] = self.get_queryset().all()[:50]
        return context
    ...
    def form_valid(self, form):
        ...
        if form.is_valid():
            ...
            if obj:
                data['form_is_valid'] = True
                data['list'] = render_to_string(self.ajax_list_partial, context, self.request)
            else:
                data['form_is_valid'] = False
                data['html_form'] = render_to_string(self.ajax_partial, context, request=self.request)
            if self.request.is_ajax():
                return JsonResponse(data)
            else:
                return super().form_valid(form)
...
class AjaxSoftDeleteMixin(ObjectMixin):
    subapp =''
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['object_list'] = self.get_queryset().all()[:25]
    ...
    def post(self, *args, **kwargs):
        if self.request.is_ajax():
            ...
            self.object.user = self.request.user.get_username()
            self.object.save()
            myPage = '/' + self.app.replace('sales/sales','sales/order/')
...
class FormMixin:
    def form_valid(self, form):
        ...
        if 'continue' in self.request.POST and 'inactive' in request.path:
            return redirect(reverse_lazy("{}:{}-inactive".format(app, model_name), kwargs={"pk":obj.pk}))
